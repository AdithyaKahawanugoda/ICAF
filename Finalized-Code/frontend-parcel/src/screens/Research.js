...
const Research = (props) => {
  ...
  useEffect(() => {
    const getResarchPapers = async () => {
      try {
        await axios
          .get(
            "http://localhost:6500/grid/api/researcherpvt/researchPapers"
          )
          .then((res) => {
  ...
  return (
    <div>
      <img
        src="https://i.ibb.co/MRQ6577/Whats-App-Image-2021-06-27-at-3-51-27-PM.jpg"
        alt="research-resource-page-img"
        border="0"
        width="100%"
      />
      {resPaper
        .filter((wrk) => wrk._id === rID)
        .map((research, index) => (
          <div>
            <ListGroup style={{ margin: "5rem" }}>
              <ListGroup.Item>
                <h3> {research.researchTopic}</h3>{" "}
              </ListGroup.Item>
              <ListGroup.Item>
                Subject: {research.researchSubject}
              </ListGroup.Item>
              <ListGroup.Item>Authors: {research.paperAuthors}</ListGroup.Item>
              <ListGroup.Item>
                Abstract: <br></br> <p>{research.paperAbstract}</p>
              </ListGroup.Item>
              <ListGroup.Item>
