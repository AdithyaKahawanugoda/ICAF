  ...
  useEffect(() => {
    const getWorkshopProposals = async () => {
      try {
        await axios
          .get(
            "http://localhost:6500/grid/api/workshopconductorpvt/workshopconductor/proposals"
          )
          .then((res) => {
  ...
  return (
    <div>
      <img
        ...
        alt="workshop-resource-page-img"
        border="0"
        width="100%"
      />
      {workshopProposal
        .filter((wrk) => wrk._id === wId)
        .map((workshop, index) => (
          <div>
            <ListGroup style={{ margin: "5rem" }}>
              <ListGroup.Item>
                <h3> {workshop.workshopTopic}</h3>{" "}
              </ListGroup.Item>
              <ListGroup.Item>
                Description: {workshop.workshopDescription}
              </ListGroup.Item>
              <ListGroup.Item>
                <Button variant="danger" href={workshop.proposalSecURL}>
